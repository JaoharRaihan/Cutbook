...
exports.resetPassword = (0, https_1.onCall)({
    ...
    const otpSnap = await db
        ...
        .get();
    if (otpSnap.empty) {
        throw new https_1.HttpsError('failed-precondition', 'No verified OTP found. Please complete phone verification first.');
    }
    const otpDoc = otpSnap.docs[0];
    const otpData = otpDoc.data();
    // verifiedAt is a Firestore Timestamp or server-set value
    const verifiedAt = otpData.verifiedAt && otpData.verifiedAt.toMillis ? otpData.verifiedAt.toMillis() : 0;
    if (verifiedAt < tenMinutesAgo) {
        throw new https_1.HttpsError('failed-precondition', 'OTP verification has expired. Please start over.');
    }
